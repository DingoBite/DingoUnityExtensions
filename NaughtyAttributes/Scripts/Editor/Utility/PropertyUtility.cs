...
namespace NaughtyAttributes.Editor
{
    ...
    {
        private static readonly Dictionary<object, Dictionary<string, Action>> _cachedOnValueChangeFunctions = new();
        ...
        public static void CallOnValueChangedCallbacks(SerializedProperty property)
        {
            ...
            {
                var cached = _cachedOnValueChangeFunctions.TryGetValue(target, out var dictionary);
                if (!cached)
                {
                    dictionary = new Dictionary<string, Action>();
                    _cachedOnValueChangeFunctions[property.serializedObject] = dictionary;
                }
                else if (dictionary.TryGetValue(onValueChangedAttribute.CallbackName, out var action))
                {
                    action?.Invoke();
                    return;
                }

                var cachedAction = (Action)Delegate.CreateDelegate(typeof(Action), target, onValueChangedAttribute.CallbackName);
                dictionary[onValueChangedAttribute.CallbackName] = cachedAction;
                cachedAction.Invoke();
            }
        }
